...
Metals News is an **MCP server** (`http://localhost/metals/public/mcp`, registered in `mcp_servers` as "Metals News"). The LLM reaches it through the MCP tools loader like every other MCP server; there is no backend Functions class for it any more (the former `MetalsNewsFunctions.php` was removed on 2026-09-06 after being commented out in favour of the MCP server).
