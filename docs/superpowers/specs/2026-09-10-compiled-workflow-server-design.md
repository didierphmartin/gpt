...
- **One server per compiled folder, one run at a time.** The generated server serialises runs: the event sink is a module global, and the playbook's gate tools are sync callables LangChain runs in an executor, which `loop.run_in_executor` does not carry a `ContextVar` into — so per-run sink isolation would not reach the code that needs it. A second run of the same workflow queues behind the first; different workflows get different processes and ports. Revisit only if parallel runs of one workflow become a real need.
