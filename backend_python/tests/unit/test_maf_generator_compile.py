"""Port of backend/tests/Unit/MafGeneratorCompileTest.php (Phase 6, Task 3).

Passes against live PHP as-is (only structural py_compile assertions, no
literal-string assertions affected by the generator rewrite the emit-test
oracle predates).
"""
...
def test_diamond_compiles():
    rc, out = _py_compile(MAFGenerator.emitMaf(_diamond_analyzed()), 'diamond')
    assert rc == 0, f"Diamond MAF failed py_compile:\n{out}"


def test_skill_fixture_compiles():
    a = _diamond_analyzed()
    a['agents']['4']['skills'] = [{'dir': 'html'}]
    rc, out = _py_compile(MAFGenerator.emitMaf(a), 'skill')
    assert rc == 0, f"Skill MAF failed py_compile:\n{out}"
...
def test_mcp_fixture_compiles():
    ...
    rc, out = _py_compile(MAFGenerator.emitMaf(a), 'mcp')
    assert rc == 0, f"MCP MAF failed py_compile:\n{out}"
