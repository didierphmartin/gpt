"""Port of Providers/DeepSeekProvider.php.

DeepSeek AI Provider — OpenAI-compatible wire format, dedicated base URL,
V4 "thinking mode" and `reasoning_content` support. Line-for-line port;
PHP method names are preserved (private PHP methods become `_name`). PHP
semantics are reproduced with the helpers in app.support.phpcompat
(`php_empty` for `empty()`, `x.get(k) if ... is not None else d` for `??`,
dict-or-list for `is_array()`).
...
- Multi-turn with tool calls requires `reasoning_content` to be round-tripped
  on subsequent requests; handled in _handleStreamingResponse / _buildMessages.

DeepSeek is NOT a copy of KimiProvider/GrokProvider: `_handleToolCallsRecursive`
carries NO duplicate-call detection (`executedTools`/md5 signatures) — PHP's
handleToolCallsRecursive (DeepSeekProvider.php:504-626) has none, unlike Kimi
and Grok's. `disableThinkingForThisCall` is a per-call instance flag computed
at the top of chat() and reset in a finally block (PHP:174-177, 284-286),
covering three triggers: the agent-level 'thinking' switch, a forced
tool_choice (the API 400s forced tool_choice under thinking mode), and B3
skill turns (`options['skill_metadata']`).
"""
...
class DeepSeekProvider(
    ...
    """Port of backend/src/Providers/DeepSeekProvider.php."""
    ...
    def __init__(self, config: Configuration):
        ...
        # Set on entry to chat() based on $options, restored in a finally
        # block. Used by B3 skill turns and forced tool_choice, where the
        # model only needs to pick which tool to call — thinking adds
        # latency and (empirically) makes V4-pro spiral through redundant
        # tool calls instead of returning a clean answer.
        self.disableThinkingForThisCall: bool = False

        providerConfig = config.get('providers.deepseek', {})

        self.displayName = providerConfig.get('display_name') if providerConfig.get('display_name') is not None else 'DeepSeek'
        self.model = providerConfig.get('model') if providerConfig.get('model') is not None else 'deepseek-v4-pro'
        self.maxTokens = providerConfig.get('max_tokens') if providerConfig.get('max_tokens') is not None else 8192
    ...
    def setFunctionExecutor(self, executor: FunctionExecutorInterface) -> 'DeepSeekProvider':
        self.functionExecutor = executor
        return self

    def setUsageTracker(self, tracker: UsageTrackerInterface) -> 'DeepSeekProvider':
        self.usageTracker = tracker
        return self

    def setSSEClient(self, client: StreamingClientInterface) -> 'DeepSeekProvider':
        self.sseClient = client
        return self

    def setLogger(self, logger: DebugLogger) -> 'DeepSeekProvider':
        self.logger = logger
        return self
    ...
    def setModel(self, model: str) -> 'DeepSeekProvider':
        self.model = model
        return self
    ...
    def chat(self, message: str, conversationHistory: list | None = None, options: dict | None = None) -> dict:
        ...
        # rejects forced tool_choice in thinking mode outright ("Thinking
        # mode does not support this tool_choice"), and even for merely-
        # required choices the model is just picking which tool to call with
        # which args — thinking adds latency and, with V4-pro, induces
        # tool-call spiraling. Covers both the chat path (skill_metadata, B3
        # chip turns) and the workflow path (forced/required tool_choice).
        # Restored in the finally block so a subsequent unforced call keeps
        # thinking on.
        toolChoiceOpt = options.get('tool_choice') if options.get('tool_choice') is not None else None
        forcedToolChoice = (
            toolChoiceOpt is not None
            and toolChoiceOpt != 'auto'
            and toolChoiceOpt != 'none'
        )
        ...
        thinkingSwitch = options.get('thinking') if options.get('thinking') is not None else None
        self.disableThinkingForThisCall = (
            thinkingSwitch == 'off'
            or forcedToolChoice
            or (thinkingSwitch != 'on' and not php_empty(options.get('skill_metadata')))
        )

        try:
            systemPrompt = self.buildSystemPrompt(options)
            # Honor caller-supplied tools (e.g. workflow client-side
            # run_skill_script) regardless of functionExecutor — client
            # tools run in the browser. The gate only governs this
            # provider's OWN server tools (_getTools()).
            ...
            if not php_empty(response.get('_pending_client_tool_call')):
                ...
                return {
                    ...
                    'assistant_reasoning': response.get('_pending_assistant_reasoning') if response.get('_pending_assistant_reasoning') is not None else '',
                    'usage': {
            ...
            self.disableThinkingForThisCall = False
    ...
    def _makeRequest(self, messages: list, tools: list | None = None, streaming: bool = False, toolChoice='auto') -> dict:
        ...
        self._sendProgress("Connecting to DeepSeek API...")
        ...
        # when thinking is enabled. We always send `thinking: {type:
        # enabled}` for V4 and omit the unsupported sampling fields. Older
        # models (V3, R1) keep the legacy temperature behaviour.
        isV4 = self.model.startswith('deepseek-v4')
        ...
        if isV4:
            ...
            payload['thinking'] = {
                'type': 'disabled' if self.disableThinkingForThisCall else 'enabled',
            }
            if self.disableThinkingForThisCall:
        ...
        if self.logger:
            self.logger.logApiRequest('deepseek', self.CHAT_ENDPOINT, payload)
        ...
        try:
            if streaming:
                with self.httpClient.stream('POST', self.CHAT_ENDPOINT, headers=headers, content=jsonPayload) as response:
                    try:
                        ...
                        # read it so the handler below can quote it (Guzzle
                        # hands us the full error body on the exception).
                ...
                if self.logger:
                    self.logger.logApiResponse('deepseek', 200, 0)
                return body
            ...
            if errorBody:
                error_log('[DeepSeekProvider] HTTP ' + str(statusCode) + ' response body: ' + errorBody[:1500])

            if statusCode == 429:
                raise ProviderException.rateLimited('deepseek')
            if statusCode == 401:
            ...
            errorDetails = str(e) + " | Response: " + guzzle_body_summary(errorBody)
            ...
            # and the errorBody-driven logging above never applies.
            raise ProviderException.apiError('deepseek', str(e), 0)
    ...
    def _handleStreamingResponse(self, response) -> dict:
        ...
        # by _sendProgress() so the user sees "DeepSeek: Thinking..."
        ...
        if not php_empty(reasoningContent):
    ...
    def _handleToolCallsRecursive(
        ...
        NOTE: unlike Kimi/Grok, DeepSeek's PHP handleToolCallsRecursive
        (DeepSeekProvider.php:504-626) carries no `executedTools`
        duplicate-call detection and no async-operation `_hint` injection —
        neither exists in the PHP source, so neither is ported here.
        """
        ...
        if not php_empty(clientCalls) and php_empty(serverCalls):
            ...
            error_log("\U0001F527 [DeepSeekProvider] Client-side tool call detected; surfacing to frontend: " + dumps([n['name'] for n in normalized]))

            reasoning = assistantMessage.get('reasoning_content') if assistantMessage.get('reasoning_content') is not None else ''
            extra = {'assistant_reasoning': reasoning} if isinstance(reasoning, str) and reasoning != '' else {}
            marker = self.emitClientToolCallEvent(normalized, assistantText, extra)
    ...
    def _buildMessages(self, conversationHistory: list, newMessage: str, systemPrompt: str, imageAttachments: list | None = None) -> list:
        ...
        for msg in conversationHistory:
            ...
            if role == 'assistant' and not php_empty(msg.get('tool_calls')):
                ...
                # Thinking mode: DeepSeek 400s ("The reasoning_content in the
                # thinking mode must be passed back") when a replayed
                # tool-call turn has NO reasoning_content key, but accepts an
                # empty string. A forced-tool turn (thinking disabled, e.g.
                # the skill turn's tool_choice=required) yields no
                ...
                messages.append({
                    ...
                    'reasoning_content': reasoningContent if isinstance(reasoningContent, str) else '',
        ...
        # DeepSeek's text models don't understand images; only vision-
    ...
    def getDefaultSystemPrompt(self) -> str:
        ...
        """
        providerConfig = self.config.get('providers.deepseek', {})
    ...
    def _sendProgress(self, message: str) -> None:
        if self.sseClient:
            self.sseClient.sendProgress(f"DeepSeek: {message}")
    ...
    def buildHttpRequest(model: str, messages: list, tools: list, config: dict, maxTokens: int, temperature: float) -> dict:
        """Build an HTTP request for DeepSeek API (static method for parallel
        execution). Uses OpenAI-compatible format with DeepSeek's base URL.
        ...
        # V4 runs in thinking mode and rejects sampling params; legacy
        # models keep the OpenAI-compatible temperature behaviour.
        ...
        if not php_empty(tools):
            payload['tools'] = DeepSeekProvider.convertToolsToOpenAIFormat(tools)

        baseUrl = config['base_url'] if not php_empty(config.get('base_url')) else DeepSeekProvider.BASE_URL
        endpoint = config['chat_endpoint'] if not php_empty(config.get('chat_endpoint')) else '/chat/completions'
    ...
    def parseHttpResponse(decoded: dict) -> dict:
        ...
        return {
            ...
            'usage': DeepSeekProvider.normalizeUsage(decoded.get('usage'), 'openai'),
        }
