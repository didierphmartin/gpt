"""Port of backend/src/AgentTeam/Services/MAFGenerator.php (1394 lines).
...
`php -r` runs against the live PHP MAFGenerator (see Phase 6 Task 3 report)
-- not retyped from the PHP source by eye.

Stale-oracle note (documented in the Task 3 report): `MafGeneratorEmitTest.php`
predates a full rewrite of this generator (it asserts a `@workflow` decorator
API -- `from agent_framework import Agent, workflow`, `_make_client`,
`async def _run_node(` -- none of which exist in current MAFGenerator.php,
which emits `ProviderClients`, `WorkflowBuilder`/`Executor`/`add_edge`,
`AgentNodeExecutor`). This port matches LIVE PHP; the fresh
`test_maf_generator_emit.py` tests LIVE PHP behaviour instead of porting the
stale assertions 1:1.
"""
...
_VAR_SLUG_RE = re.compile(r'[^a-z0-9]+', re.IGNORECASE | re.ASCII)
...
class MAFGenerator:
    ...
    def generate(self, workflowId: int, userId: str | None = None) -> dict:
        ...
        try:
            ...
            error_log('[MAFGenerator] could not load provider default models: ' + str(e))
        analyzed['providerDefaultModels'] = defaults

        name = _NAME_SLUG_RE.sub('_', php_strval(analyzed['workflow']['name']))
        return {
            'filename': name.lower() + '_maf.py',
            'code': MAFGenerator.emitMaf(analyzed),
        }

    @staticmethod
    def emitMaf(analyzed: dict) -> str:
        """PHP 51-134."""
        parts: list[str] = []
        parts.append(MAFGenerator._headerBlock(analyzed))
        parts.append(MAFGenerator._banner(
        ...
        parts.append(MAFGenerator._clientFactoryBlock())
        if not php_empty(analyzed.get('usedCatalog')):
            parts.append(MAFGenerator._banner(
                'MCP SERVER REGISTRY',
                'Baked at generation time from the workflow editor config.',
                'Maps server URL -> metadata. If a server moves, update the URL here.'))
            parts.append('MCP_SERVERS = ' + PythonEmitHelpers.jsonToPython(analyzed['usedServers'], True))
            parts.append(MAFGenerator._banner(
                'TOOL CATALOG',
            ...
            parts.append('TOOL_CATALOG = ' + PythonEmitHelpers.jsonToPython(analyzed['usedCatalog'], True))
            parts.append(MAFGenerator._banner(
                'MCP CLIENT + TOOL WRAPPERS',
            ...
            parts.append(PythonEmitHelpers.mcpClientBlock())
            parts.append(MAFGenerator._mcpToolBuilderBlock(analyzed))
            parts.append('catalog = build_tools_from_catalog()')
        else:
        ...
        if needsSkills:
            ...
            parts.append(MAFGenerator._skillRunnerBlock())
        parts.append(MAFGenerator._banner(
            'FROZEN NODE METADATA + EXECUTORS',
        ...
        parts.append(MAFGenerator._orchestrationBlock(analyzed))
        parts.append(MAFGenerator._entryBlock())
        return '\n\n'.join(parts) + '\n'

    @staticmethod
    def skillRunnerBlockForTest() -> str:
        """PHP 137. Test seam: exposes skillRunnerBlock() output for unit testing."""
        return MAFGenerator._skillRunnerBlock()
    ...
    @staticmethod
    def _mcpToolBuilderBlock(analyzed: dict) -> str:
        """PHP 162-350. Ported from ADKGenerator::adkToolBuilderBlock() with ONE
        change: catalog entries map to the PLAIN function (MAF auto-wraps
        callables), NOT FunctionTool(_tool_x)."""
        functions: list[str] = []
        ...
        for toolName, spec in analyzed['usedCatalog'].items():
            ...
            catalogEntries.append('        ' + pyToolName + ': ' + fnName)

        out = '''import contextvars
...
'''
    ...
    def _headerBlock(analyzed: dict) -> str:
        ...
        docBody = esc(PythonEmitHelpers.workflowDocBlock({
            ...
            'data_flow': MAFGenerator._dataFlowDoc(),
            'run': {
    ...
    @staticmethod
    def _clientFactoryBlock() -> str:
        """PHP 457-568."""
        return '''class ProviderClients:
    ...
    def _skillRunnerBlock() -> str:
        """PHP 585-906. skillFsSyncBlock() + the MAF-specific SkillRuntime class."""
        mafSpecific = r'''
...
class SkillRuntime:
    ...
    def _agentsBlock(analyzed: dict) -> str:
        """PHP 909-1229. Bake AGENTS metadata dict + the per-node async runner
        + fan-in helper."""
        entries: list[str] = []
        defaultModels = analyzed.get('providerDefaultModels') or {}
        ...
        for id_, ag in analyzed['agents'].items():
            id_ = php_strval(id_)
            if id_ in docById:
                entries.append(PythonEmitHelpers.nodeCommentBlock(docById[id_], '    '))
            instr = php_strval(_coalesce(ag.get('systemPrompt'), ''))
            tools = []
            for t in (ag.get('tools') or []):
                t = php_strval(t)
                if t.startswith('mcp_'):
                    t = t[4:]
                tools.append('catalog.get(' + PythonEmitHelpers.pyStr(t) + ')')
            toolsPy = '[' + ', '.join(t for t in tools if t) + ']'
            skillsPy = PythonEmitHelpers.jsonToPython(ag.get('skills') or [])
            ...
            if model == '':
                model = php_strval(_coalesce(defaultModels.get(canon), defaultModels.get(prov), ''))
            ...
            entries.append(
                ...
                + '"tools": ' + toolsPy + ', '
                + '"skills": ' + skillsPy + ', '
                + '"max_tokens": ' + str(php_intval(_coalesce(ag.get('max_tokens'), 4096))) + ', '
                + '"temperature": ' + _json_num(float(_coalesce(ag.get('temperature'), 0.7))) + ', '
                + '"thinking": ' + (PythonEmitHelpers.pyStr(thinking) if thinking in ('on', 'off') else 'None') + '},'
            )
        agents = 'AGENTS = {\n' + '\n'.join(entries) + '\n}'

        runner = r'''class ProgressReporter:
...
class OutputNodeExecutor(Executor):
    ...
    async def on_parent_output(self, msg: NodeMessage, ctx: WorkflowContext[Never, str]) -> None:
        ...
        agentsDoc = (
            '# Frozen per-node metadata from the visual editor (one entry per agent\n'
    ...
    def _globalsBlock(analyzed: dict) -> str:
        """PHP 1234-1246. MUST be emitted before orchestrationBlock because
        DEFAULT_PROMPT is used as a default parameter value in `async def
        main`'s signature."""
        ...
        folder = PythonEmitHelpers.pyStr(php_strval(analyzed.get('outputFolder'))) if analyzed.get('outputFolder') else 'None'
        return (
            'DEFAULT_PROMPT = ' + sp + '\n'
    ...
    @staticmethod
    def _orchestrationBlock(analyzed: dict) -> str:
        """PHP 1254-1336. create_workflow(): the editor canvas reconstructed
        1:1 on MAF's graph API."""
        startId = php_strval(_coalesce(analyzed.get('startNodeId'), ''))
        outId = ''
        for id_, node in analyzed['byId'].items():
            t = node.get('type') if node.get('type') is not None else (node.get('config') or {}).get('type', '')
            if t == 'output':
                outId = php_strval(id_)
                break

        varsMap: dict[str, str] = {startId: 'start_node', outId: 'output_node'}
        used = {'start_node': True, 'output_node': True}
        displayName: dict[str, str] = {startId: 'Start', outId: 'Output'}
        for id_, ag in analyzed['agents'].items():
            id_ = php_strval(id_)
            label = php_strval(_coalesce(ag.get('name'), 'node ' + id_))
            displayName[id_] = label
            v = _VAR_SLUG_RE.sub('_', label).strip('_').lower()
            if v == '' or _LEADING_DIGIT_RE.match(v):
                v = 'node_' + id_
            if v in used:
                v += '_' + id_
            used[v] = True
        ...
        inst: list[str] = []
        edges: list[str] = []
        inst.append('    # -- nodes (one executor per canvas node) --------------------------------')
        inst.append('    start_node = StartExecutor(node_id=' + PythonEmitHelpers.pyStr(startId) + ')')
        for layer in (analyzed.get('layers') or []):
            for nid in layer:
                key = php_strval(nid)
                if key == startId or key not in varsMap:
                    continue
                pList = [PythonEmitHelpers.pyStr(php_strval(p)) for p in (parents.get(key) or parents.get(nid) or [])]
                pPy = '[' + ', '.join(pList) + ']'
                if key == outId:
                    inst.append('    output_node = OutputNodeExecutor(parents=' + pPy + ')')
                else:
                    inst.append(
                        '    ' + varsMap[key] + ' = AgentNodeExecutor(node_id=' + PythonEmitHelpers.pyStr(key)
                        + ', parents=' + pPy + ', id=' + PythonEmitHelpers.pyStr(varsMap[key]) + ')'
                        + '  # "' + displayName[key].replace('"', "'") + '"'
                    )
        edges.append('    # -- edges (one add_edge per drawn connection) ---------------------------')
        for layer in (analyzed.get('layers') or []):
            for nid in layer:
                key = php_strval(nid)
                if key == startId or key not in varsMap:
                    continue
                for p in (parents.get(key) or parents.get(nid) or []):
                    pKey = php_strval(p)
                    if pKey not in varsMap:
                        continue
                    edges.append(
                        '    builder.add_edge(' + varsMap[pKey] + ', ' + varsMap[key] + ')'
                        + '  # ' + _coalesce(displayName.get(pKey), pKey).replace('"', "'")
                        + ' -> ' + _coalesce(displayName.get(key), key).replace('"', "'")
                    )
    ...
    @staticmethod
    def _entryBlock() -> str:
        """PHP 1339-1393. __main__ entry: read prompt from CLI args or
        DEFAULT_PROMPT, run, optionally save."""
        return '''# ============================== CLI ENTRY =================================
